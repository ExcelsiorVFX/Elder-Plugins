...
public class Title {
    ...
    public static void sendTitle(Player p, String title) {
        ...
        ((CraftPlayer)p).getHandle().playerConnection.sendPacket(new ProtocolInjector.PacketTitle(ProtocolInjector.PacketTitle.Action.TITLE,
                        ChatSerializer.a("{\"text\": \"\"}").a(title)));
    }
 
    public static void sendSubTitle(Player p, String subtitle) {
            if (!(((CraftPlayer)p).getHandle().playerConnection.networkManager.getVersion() >= VERSION)) return;
        ((CraftPlayer)p).getHandle().playerConnection.sendPacket(new ProtocolInjector.PacketTitle(ProtocolInjector.PacketTitle.Action.SUBTITLE,
    ...
    public static void sendTimings(Player p, int fadeIn, int stay, int fadeOut) {
            ...
            try {
                    ...
                    Object packet = PacketTitle.class.getConstructor(PacketTitle.Action.class, int.class, int.class, int.class)
                                .newInstance(PacketTitle.Action.TIMES, fadeIn, stay, fadeOut);
